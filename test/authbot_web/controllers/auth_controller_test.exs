defmodule AuthbotWeb.AuthControllerTest do
  ...
  describe "discord oauth callback" do
    test "saves name and discord user id and redirects to next step", %{conn: conn} do
      ...
      assert conn |> get_session(:member_id) == 12345678
      assert conn |> get_session(:discord_name) == "TestDiscordUser"
    end
  end

  describe "goonfleet oauth callback" do
    test "renames GSF user with ticker and redirects to next step", %{conn: conn} do
      insert(:config, guild_id: 12345, gsf_role_id: 12, ally_role_id: 234, verified_role_id: 567, alliance_ticker: true)
      with_mocks([
        {
          Authbot.BotConsumer, [],
          [
        ...
        conn =
          ...
          |> Plug.Conn.put_session(:guild_id, 12345)
          |> Plug.Conn.put_session(:member_id, 9876)
          |> get(~p"/auth/goonfleet/callback")

        assert_called Authbot.BotConsumer.give_role(12345, 9876, 12)
        assert_called Authbot.BotConsumer.change_nickname(12345, 9876, "[CONDI] TestGSFUser")
        assert redirected_to(conn) == ~p"/finished"
      end
    end

    test "renames GSF user without ticker and redirects to next step", %{conn: conn} do
      insert(:config, guild_id: 12345, gsf_role_id: 12, ally_role_id: 234, verified_role_id: 567, alliance_ticker: false)
      with_mocks([
        {
          Authbot.BotConsumer, [],
          [
        ...
        assert_called Authbot.BotConsumer.give_role(12345, 9876, 12)
        assert_called Authbot.BotConsumer.change_nickname(12345, 9876, "TestGSFUser")
        assert redirected_to(conn) == ~p"/finished"
      end
    end
    ...
    test "renames ally user with ticker and redirects to next step", %{conn: conn} do
      ...
      with_mocks([
        ...
        assert_called Authbot.BotConsumer.give_role(12345, 9876, 234)
        assert_called Authbot.BotConsumer.change_nickname(12345, 9876, "[5IGMA] TestSigmaUser")
        assert redirected_to(conn) == ~p"/finished"
      end
    end
  end
